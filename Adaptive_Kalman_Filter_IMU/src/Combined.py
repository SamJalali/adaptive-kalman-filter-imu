...
Q_hat_plus = 0.001 * identity(2)
R_hat_plus = 0.1
norm_error = 0.001
k = float(0)
vector_error = array([[0.001], [0], [0]])
EE = []
XX = []
PP = []
Lambda = []
...
time_step = 0.08  # Time step between updates (in seconds)
...
while time.time() - start_time < run_time:
    ...
    ######################### forgetting factor##########################
    k = (mean(vector_error_old) + 2 * power(error, 2)) / mean(vector_error_old)
...
savetxt('Estimatedstep1.csv', XX, delimiter=',')
savetxt('sensorstep1.csv', z_kk, delimiter=',')
savetxt('P1.csv', PP, delimiter=',')
savetxt('R1.csv', RR, delimiter=',')
savetxt('Q1.csv', QQ, delimiter=',')
savetxt('Lambda1.csv', Lambda, delimiter=',')
savetxt('U1.csv', UU, delimiter=',')
# savetxt('Acc_roll_estimation.csv',z_kk,delimiter=',')

# Close port
portHandler.closePort()
savetxt("time1.csv",time_list, delimiter=',')
savetxt("Encoder1.csv",position_list, delimiter=',')
# Plot the recorded data


plt.figure(1)
plt.plot(time_list, z_kk, color='blue', linewidth=1.5)
...
plt.grid()
plt.legend(['Sensor data','Encoder', 'AKF Estimated'])
